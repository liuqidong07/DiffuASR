...
from models.traditional_models import RandomModel, RandomSeqModel
...
def main():
    ...
    if args.model_name == 'sasrec_reverse':
        model = SASRec(user_num, item_num, device, args)
    elif args.model_name == 'bert4rec':
        model = Bert4Rec(user_num, item_num, device, args)
    elif args.model_name == 'random':
        model = RandomModel(user_num, item_num, device, args)
    elif args.model_name == 'random_seq':
        model = RandomSeqModel(user_num, item_num, device, args)
    else:
        raise ValueError
