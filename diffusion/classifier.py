...
class BPRLoss(nn.Module):
    ...
    def forward(self, pos_logit, neg_logit):
        ...
        loss = - torch.log(torch.sigmoid(pos_logit - neg_logit))
...
class BertClassifier(nn.Module):
    ...
    def log2feats(self, seqs, positions):
        ...
        timeline_mask = torch.zeros((seqs.shape[0], seqs.shape[1]))
    ...
    def forward(self, log_seqs, guide, mask_emb): # for training        
        ...
        log_seqs = torch.cat([log_seqs, mask_emb], dim=1)
        positions = torch.arange(1, log_seqs.shape[1]+1, device=self.dev).unsqueeze(0)
        ...
        if self.guide_type == 'item':
            pos_logits = torch.mul(final_feat, guide).sum(dim=-1) # (bs, 1)
            ...
            mask_guide = torch.sum(guide > 0, dim=-1) > 0   # guide: (bs, seq_len, hidden_size), mask_guide: (bs, seq_len)
            final_feat = final_feat.unsqueeze(1).repeat(1, guide.shape[1], 1)    # (bs, hidden_size) --> (bs, seq_len, hidden_size)
            pos_logits = torch.mul(final_feat, guide).sum(dim=-1) # (bs, seq_len)
            pos_labels = torch.ones(pos_logits.shape, device=self.dev)
            pos_loss = self.loss_func(pos_logits, pos_labels) * mask_guide
            loss = torch.sum(pos_loss, dim=-1) / torch.sum(mask_guide, dim=-1)
        
        elif self.guide_type == 'bpr':
            
            pos, neg = guide
            mask_guide = torch.sum(pos > 0, dim=-1) > 0
            final_feat = final_feat.unsqueeze(1).repeat(1, pos.shape[1], 1)    # (bs, hidden_size) --> (bs, seq_len, hidden_size)
            pos_logits = torch.mul(final_feat, pos).sum(dim=-1) * mask_guide # (bs, seq_len)
            neg_logits = torch.mul(final_feat, neg).sum(dim=-1) * mask_guide
            loss = torch.mean(self.loss_func(pos_logits, neg_logits), dim=-1) / torch.sum(mask_guide, dim=-1)
